...
void* thread_exibir_info(void* arg) {
    ...
    while (estado->jogo_ativo) {
        ...
        // Verificar tamanho mínimo da tela
        if (max_y < 25 || max_x < 80) {
            clear();
            attron(COLOR_PAIR(4) | A_BOLD);
            mvprintw(max_y/2, (max_x - 40)/2, "TERMINAL MUITO PEQUENO!");
            mvprintw(max_y/2 + 1, (max_x - 40)/2, "Minimo: 80x25 - Atual: %dx%d", max_x, max_y);
            mvprintw(max_y/2 + 2, (max_x - 40)/2, "Redimensione a janela...");
        ...
        // Seção de comandos (mover para baixo dos recursos)
        int comandos_y = recursos_y + 6;
        
        // Verificar se temos espaço suficiente na tela
        if (comandos_y + 12 < max_y - 3) {
            ...
            // Se não há espaço, mostrar versão compacta
            attron(COLOR_PAIR(6) | A_BOLD);
            mvprintw(comandos_y, status_x + 2, "COMANDOS: [T][P]");
            attroff(COLOR_PAIR(6) | A_BOLD);
            
            attron(COLOR_PAIR(7));
            mvprintw(comandos_y + 1, status_x + 2, "Ex: 1h, 2s, q=sair");
            attroff(COLOR_PAIR(7));
        }
        
        // Instruções na parte inferior (simplificada)
        attron(COLOR_PAIR(6));
        mvprintw(max_y - 2, 2, "Use os comandos ao lado -> | Pressione 'q' para sair | Ctrl+C emergencia");
        attroff(COLOR_PAIR(6));
